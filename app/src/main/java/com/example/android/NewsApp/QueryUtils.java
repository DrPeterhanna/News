    ...
    private static List<News> extractFeatureFromJson(String newsJSON) {
        ...
        try {
            ...
            for (int i = 0; i < results.length(); i++) {
                ...
                for (int oN = 0; oN < tags.length(); oN++) {

                    JSONObject tag_first_object = tags.getJSONObject(oN);
                    String first_name = tag_first_object.getString("firstName");
                    String last_name = tag_first_object.getString("lastName");
                    String author = first_name + " " + last_name;

                    // Create a new {@link News} object with the title, sectionName, date,
                    // and url from the JSON response.
                    News article = new News(title, sectionName, date, url, author);
